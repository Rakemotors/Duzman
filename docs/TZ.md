...
Обрати внимание: А.6 и А.7 сохраняют per-asset порог в форме "X (BTC) или Y (ETH)". Это работает в pattern engine через `applies_to: [BTC, ETH]` плюс одну формулировку условия с асимметричным порогом — реализация будет уточнена в спеке pattern_config_loader. В этой версии ТЗ их НЕ расщепляем (в отличие от А.3 и А.4), потому что для А.6/А.7 различие меньше по характеру (количественное между двумя активами одного класса), а для А.3/А.4 — качественное между классами активов.
