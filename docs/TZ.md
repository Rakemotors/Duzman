# Duzman — Техническое задание v1.3

**Персональный crypto metrics monitor. Этап А.**

Версия 1.3 · 16 мая 2026
...
Версия 1.3 — рабочая версия после дней 1-3 реализации. Включает фактическое разделение dev/prod, процесс изменения ТЗ, и формализацию работы нескольких AI-агентов.
...
Восемь шаблонов. Полное описание см. в Приложении А.

- Leveraged long buildup (WARNING)
- Leveraged short buildup (WARNING)
- Capitulation candidate (CRITICAL)
- Distribution top candidate (CRITICAL)
...
- Cooldown по `dedup_key` (pattern + asset) — настраивается per pattern
- Глобальный лимит: не более 3 алертов в час суммарно
- CRITICAL обходят глобальный лимит, но не hard cap «10 алертов в час»
...
- Hard caps на алерты (раздел 0.2)
- Тестирование 8 шаблонов на исторических данных
...
- Все 8 шаблонов проверяются каждый час
...
| Шаблоны | 8 стартовых из Приложения А |
...
Восемь шаблонов. Default cooldown 2 часа, если не указан явно.
...
### А.3. Capitulation candidate (CRITICAL, все 6 активов, cooldown 4h)

- RSI 1d < 30
- Funding средний < -0.05%
- Ликвидации longs 24h > $100M (BTC/ETH) или $20M (остальные)
- Fear&Greed < 25

### А.4. Distribution top candidate (CRITICAL, все 6, cooldown 4h)

- RSI 1d > 70
- Funding средний > +0.05%
- Ликвидации shorts 24h > $100M (BTC/ETH) или $20M (остальные)
- Fear&Greed > 75

### А.5. Funding dislocation (WARNING, все 6, cooldown 4h)
...
Версия 1.3 — рабочая версия после первых дней реализации. Она фиксирует фактический ход разработки, формализует работу нескольких AI-агентов и устанавливает правило: ни один агент не отступает от ТЗ без явного обновления документа.
